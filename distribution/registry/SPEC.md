...
| Phase 8 Apply | Draft SPEC + `Execute` / `Inspect`; [`acquire/APPLY.md`](../acquire/APPLY.md); no concurrency yet |

The next remaining work is **8 — Apply** after inspection: concurrency, partial apply, rollback. The contract is [`acquire/APPLY.md`](../acquire/APPLY.md). Do not rewrite `package:install`. Translation of `registry.Result` → unique module queries stays [`acquire/SPEC.md`](../acquire/SPEC.md).
