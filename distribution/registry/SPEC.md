...
| Phase 8 Apply | Draft SPEC + `Execute` / `Inspect` / per-root lock; [`acquire/APPLY.md`](../acquire/APPLY.md); no partial apply yet |

The next remaining work is **8 — Apply** after concurrency: partial apply, then rollback. The contract is [`acquire/APPLY.md`](../acquire/APPLY.md). Do not rewrite `package:install`. Translation of `registry.Result` → unique module queries stays [`acquire/SPEC.md`](../acquire/SPEC.md).
