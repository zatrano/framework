...
| Phase 8 Apply | Draft SPEC + `Execute` / `Inspect` / per-root lock / `ExecuteTargets`; [`acquire/APPLY.md`](../acquire/APPLY.md); rollback not yet |

The next remaining work is **8 — Apply** after partial apply: rollback. The contract is [`acquire/APPLY.md`](../acquire/APPLY.md). Do not rewrite `package:install`. Translation of `registry.Result` → unique module queries stays [`acquire/SPEC.md`](../acquire/SPEC.md).
