...
`channel: main` is the **source/development stream** of a Go module (typically the default git branch). It is **not** a published release version. A marketplace or UI “Latest / stable” label must show a tagged version (for official packages, currently `v1.12.0`) — never present `main` as a release.
...
HTTP registry service, GOPROXY as a ZATRANO protocol, dry-run, a CLI acquisition command, connecting acquisition and enablement, publisher identity, licenses, yank/retract beyond skipping a version in the index. Existing `package:enable` / `package:install` remain enablement.
