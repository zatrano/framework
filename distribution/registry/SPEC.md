...
`channel: main` is the **source/development stream** of a Go module (typically the default git branch). It is **not** a published release version. A marketplace or UI “Latest / stable” label must show a tagged version (for official packages, currently `v1.10.0`) — never present `main` as a release.
