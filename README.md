...
<p align="center">
  ...
  <a href="VERSION"><img src="https://img.shields.io/badge/version-2.5.0-green.svg" alt="Version"></a>
...
```bash
go install github.com/zatrano/framework/v2/cmd/zatrano@v2.5.0
zatrano new myapp
cd myapp
go mod tidy
...
That updates `bootstrap/enabled.go`, writes a blank-import in `bootstrap/addons.go`, `go get`s `github.com/zatrano/packages@v1.9.0` when that module is not yet required, and merges env keys into `.env.example`. Then rebuild/restart.
...
**Framework `v2.5.0`** is the current public kernel. **Packages `v1.9.0`** is the current public official-packages release. Create applications with `zatrano new`. Do not clone this repository as your application.
...
Nested modules (SQL drivers, `mongo`, `webauthn`, `qr`) are separately versioned Go modules. Their tags follow the nested path (`database/driver/sqlite/v1.0.0`), not a `packages/` prefix. Nested publication is a separate release operation. Root `packages@v1.9.0` does not require those drivers.

Historical `packages@v1.7.0` required an unpublished nested SQLite module. Do not retag it. New apps use `v1.9.0`.

| Line | Meaning |
| --- | --- |
| Framework `v2.5.0` | Current kernel / CLI / contracts |
| Packages `v1.9.0` | Current official package ecosystem |
