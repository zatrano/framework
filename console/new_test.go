...
func TestFrameworkGoModVersion(t *testing.T) {
	...
	if got := frameworkGoModVersion("2.0.0-dev"); got != "v2.0.10" {
...
func assertGeneratedFrameworkRequire(t *testing.T, modText string) {
	t.Helper()
	if !strings.Contains(modText, "github.com/zatrano/framework/v2 v2.0.10") {
		t.Fatalf("generated require must be v2.0.10:\n%s", modText)
	}
}
