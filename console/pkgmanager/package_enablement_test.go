...
func TestDisableDoesNotCallStop(t *testing.T) {
	body, err := os.ReadFile("package_enable.go")
	if err != nil {
		t.Fatal(err)
	}
	src := string(body)
	...
	end := strings.Index(src[idx:], "func rejectEnableTarget")
	if end < 0 {
		end = len(src) - idx
	}
...
func TestPackageDoctorFrameworkVersionFromGoMod(t *testing.T) {
	dir := t.TempDir()
	mod := "module example.com/app\n\nrequire github.com/zatrano/framework/v2 v" + consolecore.CurrentRelease + "\n"
	...
	if fw.Level != "OK" || !strings.Contains(fw.Message, consolecore.CurrentRelease) || !strings.Contains(fw.Message, "go.mod") {
