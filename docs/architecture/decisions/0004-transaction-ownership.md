...
Doctor (HIGH-CONFIDENCE): `orm.Transaction` / `QueryTx` in files under `app/http/controllers/{web,api,admin}` (APP-CTL-005), including helpers in those files. Cross-package helpers called from a controller are **SEMANTIC** (doctor boundary). Console commands may start a transaction. Controllers must not.
