...
func TestFreshApplicationErgonomics(t *testing.T) {
	...
	if !strings.Contains(text, "github.com/zatrano/framework/v2 v2.7.0") {
		t.Fatalf("go.mod must require v2.7.0:\n%s", text)
	}
	...
	if !strings.Contains(verOut, "2.7.0") {
		t.Fatalf("version must report 2.7.0:\n%s", verOut)
	}
