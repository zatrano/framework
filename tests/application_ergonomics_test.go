...
func TestFreshApplicationErgonomics(t *testing.T) {
	...
	if !strings.Contains(text, "github.com/zatrano/framework/v2 v2.5.1") {
		t.Fatalf("go.mod must require v2.5.1:\n%s", text)
	}
	...
	if !strings.Contains(verOut, "2.5.1") {
		t.Fatalf("version must report 2.5.1:\n%s", verOut)
	}
