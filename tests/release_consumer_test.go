...
func TestReleaseMetadata(t *testing.T) {
	...
	if version != "2.6.0" {
		t.Fatalf("VERSION=%q want 2.6.0", version)
	}
	...
	if !strings.Contains(text, "## 2.6.0 - 2026-09-15") {
		t.Fatal("CHANGELOG.md must record 2.6.0")
	}
	...
	released := strings.Index(text, "## 2.6.0")
	prev := strings.Index(text, "## 2.5.1")
	if unreleased < 0 || released < 0 || prev < 0 || !(unreleased < released && released < prev) {
		t.Fatal("CHANGELOG must keep Unreleased, then 2.6.0, then immutable 2.5.1")
	}
	...
	if !strings.Contains(string(readme), "version-2.6.0") {
		t.Fatal("README badge must show 2.6.0")
	}
}
...
func TestFreshConsumerLocalReplace(t *testing.T) {
	...
	if !strings.Contains(string(mod), "github.com/zatrano/framework/v2 v2.6.0") {
		t.Fatalf("generated go.mod must require v2.6.0:\n%s", mod)
	}
...
func TestPublishedModuleConsumption(t *testing.T) {
	...
	get := exec.Command("go", "get", "github.com/zatrano/framework/v2@v2.6.0")
	get.Dir = dir
	get.Env = append(os.Environ(), "GOTOOLCHAIN=local")
	gout, err := get.CombinedOutput()
	if err != nil {
		t.Skipf("v2.6.0 is not on the module proxy yet (tag/push not done): %v\n%s", err, gout)
	}
	...
	if !strings.Contains(string(mod), "github.com/zatrano/framework/v2 v2.6.0") {
		t.Fatalf("consumer go.mod must pin v2.6.0:\n%s", mod)
	}
}
