...
func TestReleaseMetadata(t *testing.T) {
	...
	if !strings.Contains(text, "## 2.0.28 - 2026-09-08") {
		t.Fatal("CHANGELOG.md must record 2.0.28")
	}
	unreleased := strings.Index(text, "## Unreleased")
	released := strings.Index(text, "## 2.0.28")
	prev := strings.Index(text, "## 2.0.27")
	if unreleased < 0 || released < 0 || prev < 0 || !(unreleased < released && released < prev) {
		t.Fatal("CHANGELOG must keep Unreleased, then 2.0.28, then immutable 2.0.27")
	}
	...
	if !strings.Contains(string(readme), "version-2.0.28") {
		t.Fatal("README badge must show 2.0.28")
	}
}
...
func TestFreshConsumerLocalReplace(t *testing.T) {
	...
	defer cancel()
	cmd := exec.CommandContext(ctx, "go", "run", "./cmd/zatrano", "new", dest, "--module", "example.com/freshapp", "--minimal", "--replace", root)
	cmd.Dir = root
...
func TestPublishedModuleConsumption(t *testing.T) {
	...
	get := exec.Command("go", "get", "github.com/zatrano/framework/v2@v2.0.28")
	get.Dir = dir
	get.Env = append(os.Environ(), "GOTOOLCHAIN=local")
	gout, err := get.CombinedOutput()
	if err != nil {
		t.Skipf("v2.0.28 is not on the module proxy yet (tag/push not done): %v\n%s", err, gout)
	}
	...
	if !strings.Contains(string(mod), "github.com/zatrano/framework/v2 v2.0.28") {
		t.Fatalf("consumer go.mod must pin v2.0.28:\n%s", mod)
	}
}
