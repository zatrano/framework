...
func TestReleaseMetadata(t *testing.T) {
	...
	if !strings.Contains(text, "## 2.3.1 - 2026-09-11") {
		t.Fatal("CHANGELOG.md must record 2.3.1")
	}
	...
	released := strings.Index(text, "## 2.3.1")
	prev := strings.Index(text, "## 2.3.0")
	if unreleased < 0 || released < 0 || prev < 0 || !(unreleased < released && released < prev) {
		t.Fatal("CHANGELOG must keep Unreleased, then 2.3.1, then immutable 2.3.0")
	}
	...
	if !strings.Contains(string(readme), "version-2.3.1") {
		t.Fatal("README badge must show 2.3.1")
	}
}
...
func TestPublishedModuleConsumption(t *testing.T) {
	...
	get := exec.Command("go", "get", "github.com/zatrano/framework/v2@v2.3.1")
	get.Dir = dir
	get.Env = append(os.Environ(), "GOTOOLCHAIN=local")
	gout, err := get.CombinedOutput()
	if err != nil {
		t.Skipf("v2.3.1 is not on the module proxy yet (tag/push not done): %v\n%s", err, gout)
	}
	...
	if !strings.Contains(string(mod), "github.com/zatrano/framework/v2 v2.3.1") {
		t.Fatalf("consumer go.mod must pin v2.3.1:\n%s", mod)
	}
}
