...
func TestAgentsGenerateIdempotent(t *testing.T) {
	...
	t.Cleanup(func() { version.Set(old) })
	if _, err := WriteAgentsMarkdown(dir); err != nil {
		t.Fatal(err)
	}
	c, err := os.ReadFile(p1)
	if err != nil {
		t.Fatal(err)
	}
	if string(c) == string(a) {
		t.Fatal("expected AGENTS.md to change when framework version changes")
	}
	if !strings.Contains(string(c), "9.9.9-test") {
		t.Fatalf("version not reflected:\n%s", c)
	}
}
