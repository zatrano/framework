...
- Phase 4.5: `unique` / `exists` fail closed when the PresenceChecker is missing or the database lookup cannot complete (ADR-0010 amended). Runtime lives in `github.com/zatrano/packages`. Kernel/ORM/doctor catalog unchanged. Report: [docs/architecture/phase4.5.md](docs/architecture/phase4.5.md).
- Phase 5 platform conformance audit: [docs/architecture/phase5.md](docs/architecture/phase5.md). Recommendation GO WITH CONDITIONS (tag hygiene; packages fail-closed unpublished). No architecture redesign.
