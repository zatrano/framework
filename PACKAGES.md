...
```text
github.com/zatrano/framework/v2   v2.5.1
github.com/zatrano/packages       v1.9.1
```
...
go run ./cmd/zatrano package:acquire session
go run ./cmd/zatrano package:acquire session --enable
```
...
**Heavy** packages (`mongo`, `webauthn`, `qr`) and SQL drivers use a separate Go module — enable or `db:setup` only when needed. They are not required by root `github.com/zatrano/packages@v1.9.1`.

Public nested-module tags follow Go path semantics, for example:

```text
github.com/zatrano/packages/database/driver/sqlite
tag: database/driver/sqlite/v1.0.0
```

A git tag named `packages/database/driver/sqlite/v1.0.0` is not a valid Go module version. Nested publication is a separate release operation; it is not part of the root `v1.9.1` tag. Historical `packages@v1.7.0` required that unpublished SQLite module — upgrade to `v1.7.1` then `v1.7.2` then `v1.8.0` then `v1.9.0` then `v1.9.1`; do not retag `v1.7.0`.
...
```text
Acquire → Imported → Enabled ∩ Imported → Expand Requires → Bootstrap
```

`package:acquire` mutates go.mod via Go modules. `package:enable` writes enablement + blank-imports. `Expand` is declared `Requires` metadata, not module resolution. Default acquire does not enable.
...
**For:** Markdown documentation repository (`docs.From(app)`). Navigation is `docs/navigation.json` when present.  
Docs: [Docs](https://zatrano.com/docs/docs)

### `seo`

**For:** Classic crawler SEO and LLM discovery as one addon: sitemap.xml, robots.txt, OG/JSON-LD `ViewData`, security.txt, `/llms.txt`, `/llms-full.txt`, `/.well-known/ai-plugin.json`. Optional `docs` fills sitemap and llms-full. Does not import `ai`. There is no `sitemap` or `wellknown` package.
